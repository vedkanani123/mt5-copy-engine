-- Per-account copy ON/OFF, plus nested master clusters:
-- trade on master A copies to A's followers (slaves + linked masters)
-- and recursively to those linked masters' followers. A paused account
-- is skipped; its descendants that are still ON still receive the trade.
  ...
  while coalesce(array_length(v_queue, 1), 0) > 0 loop
    ...
    for v_child in
      ...
      where workspace_id = v_master.workspace_id
        and master_account_id = v_parent
