-- Re-notify slaves even if the master event was already stored (so a missed
-- first attempt still copies). Never skip a live slave that is COPY ON.
    ...
    loop
      ...
      values (
        ...
        jsonb_build_object(
          ...
          'sl_distance', coalesce((p_event->>'sl_distance')::numeric, 0),
          'tp_distance', coalesce((p_event->>'tp_distance')::numeric, 0),
...
begin
  ...
  if v_cmd.id is not null then
    ...
    return jsonb_build_object(
      ...
      'sl', (v_payload->>'sl')::numeric,
      'tp', (v_payload->>'tp')::numeric,
